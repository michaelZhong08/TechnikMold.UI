...
function ProjectGrid(keyword, state, type, depID) {
    ...
    //var _url = '/Project/JsonProjects?keyword='+keyword+"&State="+state+"&Type="+type;
    ...
    $("#ProjectGrid").jqGrid({
        ...
        colModel: [
            ...
            {
                label: '项目号', name: 'ProjectNo', key: true, width: 75, align: "center", sorttable: false, cellattr: function (rowID, tv, rawobject, cm, rdata) { return 'id=\'ProjectNo' + rowID + "\'"; },
            },
            {
                label: '模具号', name: 'MoldNo', width: 75, sorttable: false, cellattr: function (rowID, tv, rawobject, cm, rdata) {
            ...
            { label: 'CAM', name: 'CAM', width: 75, sorttable: false, cellattr: function () { return 'id=2'; } },
            { label: '采购', name: 'Purchase', width: 75, sorttable: false, cellattr: function () { return 'id=3'; } },
            { label: '开粗', name: 'Prototype', width: 75, sorttable: false, cellattr: function () { return 'id=4'; } },
            { label: 'CNC开粗', name: 'CNCPrototype', width: 75, sorttable: false, cellattr: function () { return 'id=5'; } },
            { label: '热处理', name: 'HeatProcess', width: 75, sorttable: false, cellattr: function () { return 'id=6'; } },
            { label: '磨床', name: 'Grinding', width: 75, sorttable: false, cellattr: function () { return 'id=7'; } },
            { label: 'CNC', name: 'CNC', width: 75, sorttable: false, cellattr: function () { return 'id=8'; } },
            { label: 'EDM', name: 'EDM', width: 75, sorttable: false, cellattr: function () { return 'id=9'; } },
            { label: 'WEDM', name: 'WEDM', width: 75, sorttable: false, cellattr: function () { return 'id=10'; } },
            { label: '装配', name: 'Assembly', width: 75, sorttable: false, cellattr: function () { return 'id=11'; } },
            { label: '试模', name: 'FOT', width: 75, sorttable: false, cellattr: function () { return 'id=12'; } },
            { label: 'OTS', name: 'OTS', width: 75, sorttable: false, cellattr: function () { return 'id=13'; } },
            { label: 'PPAP', name: 'PPAP', width: 75, sorttable: false, cellattr: function () { return 'id=14'; } },
            {
                label: '备注', name: 'Memo', width: 100, sorttable: false, cellattr: function (rowID, tv, rawobject, cm, rdata) {
        ...
        loadComplete: function () {
            $(".jqgrow", this).contextMenu("ProjectContextMenu", {
                bindings: {
                    ...
                    'AddProject': function () {
                        location.href = "/Project/Edit?ParentID=" + GetCurrentID("ProjectGrid");
                    },
                    'Milestone': function () {
...
function PartListGrid(MoldID, Height) {
    $("#PartGrid").jqGrid({
        ...
        datatype: "json",
        colModel: [
            { label: 'No.', name: 'Index', width: 20 },
            { label: '', name: 'ID', width: 75, hidden: true },            
            { label: '零件名', name: 'PartName', key: true, width: 100, sorttable:true, sorttype:'Text' },
            { label: '物料编号', name: 'MaterialNo', width: 60 },
            { label: '尺寸或规格', name: 'Specification', width: 100 },
            { label: '材料', name: 'Material', width: 50, sortable: true, sorttype: "text" },
            { label: '数量', name: 'Quantity', width: 40 },
            { label: '附加数量', name: "AppendQty", width: 50 },
            { label: '库存', name: 'InStock', width: 40 },
            { label: '硬度', name: 'Hardness', width: 50 },
            { label: '品牌', name: 'Supplier', width: 50 },
            { label: '详图', name: 'DetailDrawing', formatter: "checkbox", width: 25 },
            { label: '略图', name: 'BriefDrawing', formatter: "checkbox", width:25 },
            { label: '附图订购', name: 'PUDrawing', formatter: "checkbox", width: 25 },
            { label: '追加工', name: 'AdditionalMachining', formatter: "checkbox", width: 30 },
            { label: '备注', name: 'Memo', width: 75 },
            { label: '创建日期', name: 'Drawing', width: 60 },
            { label: 'UG', name: 'FromUG', formatter: "checkbox", width: 25 },
            { label: '采购', name: 'InPurchase', formatter: "checkbox", width: 25 },
        ],
        viewrecords: true,
        height: Height,
...
function CAMTaskList(MoldNumber, TaskType, State, CAM) {
    $("#TaskGrid").jqGrid({
        ...
        colModel: [
            ...
            { label: '工艺名', name: 'ProcessName', width: 75, hidden: true },
            ...
            { label: '时间', name: 'Time', width: 75, hidden: true },
            { label: '状态', name: 'State', width: 50, hidden:true },
            ...
            { label: '数量', name: 'Quantity', width: 75, hidden: true },
            ...
            { label: '计划时间', name: 'PlanTime', width: 75 },
            ...
            $(".jqgrow", this).contextMenu("CAMTaskContextMenu", {
                bindings: {
                    ...
                    'ReleaseCAMTask': function () {
                            ReleaseCAMTask(GetMultiSelectedIDs("TaskGrid"));
                    },
    ...
    $("#TaskGrid").jqGrid({
        ...
        loadComplete: function () {
            $(".jqgrow", this).contextMenu("MachineTaskContextMenu", {
                ...
                //onContextMenu: function (event/*, menu*/) {
                //    //Set the mouse hovered line as selected status                    
                //    $("#TaskGrid").jqGrid("setSelection", $(event.target).closest("tr.jqgrow").attr("id"));
                //    return true;
                //}
            });
        },
    ...
    $("#TaskItemGrid").jqGrid({
        ...
        loadComplete: function () {
            $(".jqgrow", this).contextMenu("CNCItemContextMenu", {
                ...
                //    $("#TaskItemGrid").jqGrid("setSelection", $(event.target).closest("tr.jqgrow").attr("id"));
                //    return true;
                //}
            });
...
function EDMItemList(TaskIDs) {
    ...
    $("#TaskGrid").jqGrid({
        ...
        multiselect: true,
        ondblClickRow: function (iRow) {
            var row = GetDblClickRow();
            var _id = $("#TaskGrid").getCell(row, "ID");
            LoadElectrodeQC(_id);
        },
    })
}
...
function POContentGrid(POID) {
    ...
    $("#POContentGrid").jqGrid({
        ...
        colModel: [
            ...
            { label: '', name: 'PurchaseTypeID', hidden:true},
            { label: '采购类型', name: 'PurchaseType', width: 50 },
...
function TaskFinish(TaskType) {
    ...
    $("#TaskGrid").jqGrid({
        ...
        colModel: [
            ...
            {
                label: '开始时间', name: 'StartTime', width: 100, formatter: function (cellvalue) {
                    if (cellvalue == '1900-01-01 12:00') {
                        return "-";
                    } else {
                        return cellvalue;
                    }
