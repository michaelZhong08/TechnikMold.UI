    ...
    public class PurchaseItemRepository:IPurchaseItemRepository
    {
        ...
        public int Save(PurchaseItem PurchaseItem)
        {
            ...
            if (PurchaseItem.PurchaseItemID == 0)
            {
                ...
                if (PurchaseItem.MoldNumber != "其他采购")
                {
                    if (_context.Projects.Where(p => p.MoldNumber == PurchaseItem.MoldNumber).Where(p => p.Type == 2).Count() > 0)
                    {
                        PurchaseItem.ProcedureType = 20;
                    }
                    else
                    {
                        PurchaseItem.ProcedureType = 10;
                    }
                }
                else
                {
                    PurchaseItem.ProcedureType = 30;
                }
            ...
            else
            {
                ...
                _dbEntry.Specification = PurchaseItem.Specification==null?"":PurchaseItem.Specification;
