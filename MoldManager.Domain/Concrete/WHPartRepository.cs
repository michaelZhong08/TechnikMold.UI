...
namespace TechnikSys.MoldManager.Domain.Concrete
{
    public class WHPartRepository: IWHPartRepository
    {
        ...
        public string GetwfTaskPartNum(int _taskID)
        {
            ...
            if (_sObj != null)
            {
                ...
                else
                {
                    ...
                    string _fLetter = _sObj.NameConvension.Substring(0, 1);//Z
                    string _ZeroList = _sObj.NameConvension.Substring(1, _sObj.NameConvension.Length - 1);//0000
                    string _strNum = string.Format("{0:" + _ZeroList + "}", num);//0001
                    return _task.MoldNumber + "-" + _fLetter + _strNum;//xxxxxx-Z0001
                }
            }
            return null;
        }
        ...
        {
            List<WHPart> _wfparts = _context.WHParts.Where(p => p.MoldNumber == _moldNum && p.TaskID > 0 && p.Enable == true).ToList();
            return _wfparts;
        }
    }
}
